...
class Avatar(KBEngine.Entity,
	...
	def dropNotify(self, exposed, bagitem):
		DEBUG_MSG("Avatar::dropNotify s ")
		if exposed != self.id:
			return
		DEBUG_MSG("Avatar::dropNotify modelId is : %i", bagitem[8])
		params = {
			"bagItemData"	: bagitem,
			"modelID" : bagitem[8],
			"name" : bagitem[5],
	...
	def pickUpRequest(self, exposed, droppedItemID):
		if exposed != self.id:
			return
		item = KBEngine.entities[droppedItemID]
		if item == None:
			return
		item.pickUpRequest( self.id )
		
	def pickUpResponse(self, exposed, succuss, droppedItemID):
		if exposed != self.id:
			return
		if succuss == 0:
			item = KBEngine.entities[droppedItemID]
		...
		if succuss == 1:
			...
			item.isPicked(1)
			return
	def ItemUse(self, exposed, item):
		if exposed != self.id:
			return
		itemScript = items.getitem(item[32])
		if itemScript is None:
			ERROR_MSG("Spell::spellTarget():skillID= not found %s", item[32])
			return
		itemScript.use(self, item)
		
	def EquipNotify(self, exposed, item):
		if exposed != self.id:
			return
		itemScript = items.getitem(item[32])
		if itemScript is None:
